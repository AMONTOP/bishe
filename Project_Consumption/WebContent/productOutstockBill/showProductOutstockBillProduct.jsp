 ...
 <form action="findbyNameproduct" method="post" style="position:absolute;top:19px;right:187px;">
	   <p class="columns pull-right search-button">
	    ...
	    <p class="columns pull-right search-input">
	   		<input class="form-control" name="product.productName"  type="text" placeholder="请根据出库单编号搜索">
	   </p>
   </form>

<div id="main">
    <div id="toolbar">
        <a class="waves-effect waves-button" href="addPre2productOutstockBill"><i class="zmdi zmdi-plus"></i> 新增货品出库单货品</a>
    </div>
    <table id="table"></table>
				   	...
				   	<c:forEach items="${list}" var="productInstockBill">
				        <script>
				    	 ...
				    	 var productWhereabouts = "${productInstockBill.productWhereabouts}";
				    	 ...
				    	 var quantity = "${productInstockBill.quantity}";
...
<script>
    ...
    $(function() {
        ...
        $table.bootstrapTable({
            ...
            toolbar: '#toolbar',
            columns: [
                {field: 'billNo', title: '货品出库单编号', sortable: false, halign: 'center'},
                {field: 'productNo', title: '货品编号', sortable: false, halign: 'center'},
                {field: 'productName', title: '出库货品', sortable: false, halign: 'center'},
    ...
    function actionFormatter(value, row, index) {
        return [
            ...
            '<a class="edit ml10" href="prepup2productInstockBill?billId='+ row.id +'" data-toggle="tooltip" title="Edit"><i class="glyphicon glyphicon-edit"></i></a>　',
            '<a class="remove ml10" href="delete2productInstockBill?billId='+ row.id +'" data-toggle="tooltip" title="Remove"><i class="glyphicon glyphicon-remove"></i></a>'
        ].join('');
    }
