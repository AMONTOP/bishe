...
<script>
 ...
 <div class="pop_box hide">
                    ...
                    <div class="contentP">
                        ...
                        <div class="textmian">
                        <!-- 更改字段?????????? -->
                        	<p style="font-size: 17px;">注意：退料成功后，相对应的仓库货品将增加！</p>
                        	<c:forEach items="${list2}" var="myreturn" varStatus="status">	
	                       		<form action="saveOthermaterialHouseMaterial" method="post" id="myform${status.index}" style="text-align: center;">
	                       			  <input type="hidden" name="returnMaterialBill.returnbillId" value="${myreturn.returnMaterialBill.returnbillId}" />	                       			 	    				   							    
								     数量：<input type="text" name="mwareHouseMaterial.quantity" value="${myreturn.quantity}" />
								      <input type="hidden" name="mwareHouseMaterial.material.materialId" value="${myreturn.material.materialId}" />			        						      				    
								     物料： <input type="text" value="${myreturn.material.materialName}" readonly />		
								     仓库： <input type="text" value="${myreturn.returnMaterialBill.mwareHouse.wareNo}" readonly />
								      <input type="hidden" name="mwareHouseMaterial.mwareHouse.wareId" value="${myreturn.returnMaterialBill.mwareHouse.wareId}" />							     							     						    
...
<script>
     		...
     		(function(i){
     			$("#saveR"+i).click(
     				function(){
     				$.ajax({
						url:"saveOthermaterialHouseMaterial",
						type:'post',
						asycn:false,
