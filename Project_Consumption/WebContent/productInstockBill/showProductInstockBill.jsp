...
<script>
 ...
 <p style="text-indent:2em;color:red;background: #fafafa;margin:0;padding-top:10px">注意：可点击有绿色方框的“审核未通过”按钮代表还未将该货品加入到二次加工的列表，而不可点击的“审核未通过”则代表已经加入了二次加工列表！</p>
 <p style="text-indent:2em;color:red;background: #fafafa;margin:0">审核过程需要，点击“待审核”进入到货品品质审核，在品质审核中进行审核后，则可完成审核！</p>
<div id="main">
    <div id="toolbar">
...
<script>
	...
	function actionFormatter3(value, row, index){
		if(row.billState == "待审核"){
			return [
				...
				'<a href="insertproductInstockBill?billNo='+row.billNo+'" class="innerbtn btn btn-primary btn-lg">待审核</a>'
			].join('');
			...
			return [
				'<a href="findbyBillIdproductCheck?billId='+row.id+'" data-flag="'+row.flag+'" class="innerbtn btn btn-primary btn-lg">审核未通过</a>'
			].join('');
		}
	}
  ...
  $(document).ready(function(){
	...
	$(".mainform").attr("action","export2ExcelproductInstockBill");
