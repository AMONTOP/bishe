...
<html>
  ...
  <body>
   ...
   <div style="background: #fafafa;">
	   	<a href="showListproductCheck?id=0" class="topSelect">审批未通过</a>
		<a href="showListproductCheck?id=1" class="topSelect">审批通过</a>
   </div>
  ...
  <div id="main">
	<div id="toolbar">
		<a class="waves-effect waves-button" href="addPreproduct"><i class="zmdi zmdi-plus"></i> 新增货品品质</a>
	</div>
	<table id="table"></table>
		...
		<c:forEach items="${list}" var="productCheck">
	    	 <script>
	    	 var id = ${productCheck.productCheckId};
	    	 var checkNo = "${productCheck.checkNo}";
	    	 var productId = "${productCheck.product.productId}";
	    	 var productName = "${productCheck.product.productName}";
	    	 var empId = "${productCheck.employee.empId}";
	    	 var empName = "${productCheck.employee.userName}";
	    	 var checkItem = "${productCheck.checkItem}";
	    	 var checkNumber = "${productCheck.checkNumber}";
	    	 var cdate = "${productCheck.cdate}";
	    	 var result = "${productCheck.result}";
	    	 var note = "${productCheck.note}";
	    	 var qualify = "${productCheck.qualify}";
			 var state = "${productCheck.state}";
			 var billNo = "${productCheck.productInstockBill.billNo}";
			 if(state == "false"){
			 	state = "审批未通过";
			 }else if(state == "true"){
			 	state = "审批通过";
			 }
			 var checkState = "${productCheck.checkState}";
...
function actionFormatter(value, row, index) {
    return [
        ...
        '<a class="edit ml10" href="prepupproductCheck?productCheckId='+ row.id +'" data-toggle="tooltip" title="Edit"><i class="glyphicon glyphicon-edit"></i></a>　',
        '<a class="remove ml10" href="deleteproductCheck?productCheckId='+ row.id +'" data-toggle="tooltip" title="Remove"><i class="glyphicon glyphicon-remove"></i></a>'
    ].join('');
}

function actionFormatter2(value, row, index){
	if(row.checkState == "未检测"){
      return ['<a href="prepupproductCheck?productCheckId='+ row.id +'" class="innerbtn" style="color:blue" >' + row.checkState + '</a>'].join('');
    }else{
