...
<script>
	function randomNumber(){
		...
		console.log(hour);
		return "UM" + hour.toString() + minutes.toString() + seconds.toString() + (Math.round(Math.random() * 4)).toString();
	};
...
<script>
 ...
 <div class="pop_box hide">
                    ...
                    <div class="contentP">
                        ...
                        <div class="textmian">
                        	<p>审核未通过需要录入二次加工物料表中，等候相关人员进行二次加工物料</p>
                        	
                       		<form action="test2uselessMaterial" method="post">				    				   							    
							      <input type="hidden" name="uselessMaterial.useNo" id="useNo"/>			        
							      <input type="hidden" name="uselessMaterial.materialInstockBill.billId" id="billId" />							  			      							        
							      <input type="hidden" name="uselessMaterial.materialCheck.materialCheckId" id="materialCheckId"/>
							      <input type="hidden" name="uselessMaterial.materialCheck.checkItem" id="checkItem"/>							      				    
	...
	<c:forEach items="${list}" var="materialCheck">
                        		<script>
                        			var materialCheckId = "${materialCheck.materialCheckId}";
                        			var billId = "${materialCheck.materialInstockBill.billId}";
                        			var checkItem = "${materialCheck.checkItem}";
                        			var bill = document.getElementById("billId");
                        			bill.value = billId;
                        			var materialCheck = document.getElementById("materialCheckId");
                        			materialCheck.value = materialCheckId;
